...
| 概念 | 取值 | 出现位置 |
|---|---|---|
| 仓库 tag | `v0.3.3`（安装器/整包版本） | Release 标签、`install.sh` 的 `VERSION` |
| 应用版本 | `1.2.4`（`app/VERSION`） | 应用资产文件名、`install-dhcp.conf` 的 `VERSION` |
...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/driftbottle61/iptv-spider-pve/v0.3.3/install.sh)
```
